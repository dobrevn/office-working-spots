...
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImlleXVqYWdvZG5yYWZwYmFneXZuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODM0OTc3NDAsImV4cCI6MjA5OTA3Mzc0MH0.T2cjzuPrGgXuftzBTiQVF5zy5g1xJrwJsXaGCKAPx4Y"; // Сложи реалния си дълъг public anon ключ
...
async function fetchCloudData() {
    try {
        const { data: resData } = await supabaseClient.from('office_reservations').select('*').eq('date', selectedDateStr);
        activeReservations = {};
        if (resData) { resData.forEach(row => { activeReservations[row.desk_id] = row.user_name; }); }

        const { data: meetData } = await supabaseClient.from('office_meetings').select('*').eq('date', selectedDateStr);
        activeMeetings = {};
        if (meetData) { meetData.forEach(row => { activeMeetings[row.slot_index] = { title: row.title, attendees: row.attendees }; }); }

        renderOffice();
        renderMeetings();
    } catch (err) { console.error("Грешка при теглене:", err); }
}
...
function renderOffice() {
    ...
    tableContainers.forEach(id => { const el = document.getElementById(id); if (el) el.innerHTML = ''; });

    // Рендериране на новите вертикални позиции за Маса А
    for(let i=1; i<=4; i++) {
        const pos = document.getElementById(`pos-M-${i}`);
        if(pos) { pos.innerHTML = ''; pos.appendChild(createDeskIcon(`M-${i}`, i)); }
    }
    
    // Останалите маси в десния край
...
// УГОЛЕМЕНИ БЮРА С КРАСИВ И ЮЗЪР-ФРЕНДЛИ ДИЗАЙН (w-16 h-16)
function createDeskIcon(id, displayNum) {
    ...
    if (isOccupied) {
        ...
        wrapper.style.backgroundColor = "#10b981/10";
        wrapper.style.color = "#10b981";
        wrapper.style.borderColor = "#059669";
    }

    // Уголемен до w-16 h-16 с по-голяма икона за по-лесно цъкане
    wrapper.className = "flex flex-col items-center justify-center p-2 rounded-xl border-2 transition-all duration-200 hover:scale-110 cursor-pointer w-16 h-16 relative group shadow-md";
    wrapper.innerHTML = `
        <i class="fa-solid fa-desktop text-base"></i>
        <span class="text-[10px] font-black mt-1 font-mono">${displayNum}</span>
        <span class="absolute bottom-full mb-1 scale-0 group-hover:scale-100 transition-all text-[10px] bg-slate-950 text-slate-200 px-2.5 py-1 rounded-md border border-slate-800 font-medium whitespace-nowrap z-50 shadow-2xl">
    ...
    wrapper.onclick = async () => {
        selectedDeskId = id;
        if (isOccupied) {
            const checkMyName = cyrillicToLatin(localStorage.getItem('current_user_name') || '');
            const checkOccupant = cyrillicToLatin(occupant);
            if (checkMyName && (checkMyName === checkOccupant)) {
            ...
            } else { alert(`Грешка: Бюро ${displayNum} е запазено от ${occupant}. Не можете да триете чужди резервации!`); }
        } else {
...
function renderMeetings() {
    ...
    TIME_SLOTS.forEach((slot, index) => {
        ...
        if (isBooked) {
            card.className = "p-4 rounded-xl border border-rose-900/40 bg-rose-950/10 flex flex-col justify-between gap-2 shadow-sm";
            card.innerHTML = `
                <div>
            ...
            card.className = "p-4 rounded-xl border border-slate-800/80 bg-slate-900/10 hover:border-indigo-500/40 hover:bg-indigo-500/5 transition-all flex justify-between items-center cursor-pointer group shadow-sm";
            card.innerHTML = `
...
document.getElementById('btn-confirm-booking').onclick = async () => {
    ...
    const hasAlreadyBooked = Object.values(activeReservations).some(occupant => { return cyrillicToLatin(occupant) === newNameNormalized; });
    if (hasAlreadyBooked) {
        alert(`Грешка: Името "${name}" вече има запазено бюро за тази дата!\n\nАко това е ваш съименник, моля въведете и фамилия (напр. ${name} Иванов / ${name} I.), за да се разграничите.`);
